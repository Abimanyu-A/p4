...
2. **Scan** — runs Semgrep with broad, pattern-only rules (`backend/rules/`)
   for injection, insecure deserialization, and SSRF. This intentionally
   behaves like a naive baseline SAST tool — it casts a wide net and produces
   false positives on purpose, which is what Validate exists to fix.
